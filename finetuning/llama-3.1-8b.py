...
parser.add_argument('--data-file', type=str, default="merged_filtered_data.json", help="Path to the data file")
...
# Process the accuracy data which contains conversation examples
if "accuracy" in data_source:
    for entry in data_source["accuracy"]:
        if "messages" in entry and isinstance(entry["messages"], list) and entry["messages"]:
            # Extract conversations
            messages = entry["messages"]
            if len(messages) < 2:  # Skip if not enough messages
                continue
...
MODEL_ID = "meta-llama/Llama-3.1-8B-Instruct"  # Same as used in llm_server.py
...
def formatting_chat_func(examples):
    ...
    for example in examples["messages"]:
        ...
        valid_messages = [msg for msg in example if isinstance(msg, dict) and "content" in msg and "role" in msg]
